...
local function createTopLine(char, str, width)
    ...
    if len ~= 2 then
        returnString = returnString
            .. string.rep(char, math.floor(width / 2 - len / 2))
            .. " "
            .. str
            .. " "
...
local function fill_border_data(buf, width, height, title, border_chars)
    ...
    local border_lines = {
        border_chars.TOP_LEFT .. topLine .. border_chars.TOP_RIGHT,
    }
...
function M.create_win(opts)
    ...
    if opts.border then
        local border_win_buf_pair = create_win(
            opts.row - 1,
            opts.col - 1,
            opts.width + 2,
            opts.height + 2,
    ...
    if opts.border then
        local autocmds = {
            ["BufDelete,BufWipeout"] = string.format(
                "bwipeout! %s",
                border_buf
            ),
            ["nested"] = true,

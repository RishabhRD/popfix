...
local promptHighlightNamespace = api.nvim_create_namespace(
    "popfix.prompt_highlight"
)
...
function prompt:new(opts)
    ...
    api.nvim_buf_add_highlight(
        obj.buffer,
        promptHighlightNamespace,
        obj.prompt_highlight,
        0,
        0,
        #obj.prefix
    )
...
function prompt:registerTextChanged(func)
    ...
    if not self.attached then
        local function on_lines(_, _, _, first, last)
            local promptText = vim.api.nvim_buf_get_lines(
                self.buffer,
                first,
                last,
                false

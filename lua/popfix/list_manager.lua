...
function M:select(lineNumber, callback)
    ...
    if self.sortedList[lineNumber] then
        data = self.action:select(
            self.sortedList[lineNumber].index,
            self.sortedList[lineNumber].line,
            callback
        )
    end
    if data then
...
function M:clear()
    ...
    vim.schedule(function()
        self.list:clear()
        api.nvim_buf_clear_namespace(self.list.buffer, identifier, 0, -1)
    end)
end
...
function M:add(line, index, originalIndex)
    ...
    if self.currentPromptText ~= nil then
        highlight = self.highlightingFunction(
